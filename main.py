...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        if len(sys.argv) == 2:
            page_num = ""
            print("為偵測到目標頁數，因此只進行最新文章頁面進行爬蟲")
        else:
            page_num = sys.argv[2]
        ...
        '''爬蟲執行順序
             ...
             parser_board_urls: arg1: request class, return: (list)urls
             parser_article_content: arg1: (list) urls, return: (list) ppt_data
             export_json: arg1: (list) ppt_data, return: (Boolean) True, Output: ptt.json
        ...
        process_st = send_line_notify("{}- 看板-{} 爬蟲已完成".format("", target_board))
        if process_st == 200:
